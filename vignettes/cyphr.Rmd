...
these can also be file names, each of which contains the contents of
...
When passed a file name (as opposed to a raw vector),
...
which argument corresponds to the file name (here `"secret.rds"`).
...
detects that the first argument represents the file name.  It
...
`import` and `export` functions take the file name `file` you could
use:
