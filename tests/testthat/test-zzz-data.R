...
test_that("initialisation", {
  ...
  quiet <- FALSE
  res <- suppressMessages(data_admin_init(path, "pair1", quiet))
  ...
  msg <- capture_messages(data_request_access(path, "pair1"))
  expect_match(msg, "You appear to already have access")

  msg <- capture_messages(data_admin_init(path, "pair1", quiet))
  expect_match(msg[[1]], "Already set up")
})
...
test_that("grant access", {
  ...
  dir.create(path, FALSE, TRUE)
  quiet <- FALSE
  res <- data_admin_init(path, "pair1", quiet = TRUE)
  r <- runif(10)
  encrypt_object(r, res, file.path(path, "data.rds"))

  h <- data_request_access(path, "pair2", quiet = TRUE)
  msg <- capture_messages(h2 <- data_request_access(path, "pair2", quiet))

  expect_match(msg[[1]], "already pending")
  ...
  expect_message(keys <- data_load_request(path, NULL, quiet),
  ...
  ans <- data_admin_authorise(path, h, "pair1", TRUE, quiet = TRUE)
...
test_that("authorise no keys", {
  ...
  dir.create(path, FALSE, TRUE)
  res <- data_admin_init(path, "pair1", quiet = TRUE)
  msg <- capture_messages(data_admin_authorise(path, path_user = "pair1"))
  expect_match(msg[[1]], "There is 0 requests for access")
  expect_match(msg[[2]], "No keys to add")
})
  ...
  testthat::with_mocked_bindings(
    prompt_confirm = function() FALSE, {
      msg <- capture_messages(
        try(data_admin_authorise(path, h, "pair1", FALSE), silent = TRUE))
      expect_match(msg, "Cancelled adding key", all = FALSE)
      expect_error(
        suppressMessages(data_admin_authorise(path, h, "pair1", quiet = TRUE)),
        "Errors adding 1 key")
    })
})
...
test_that("decryption failed gives reasonable error", {
  ...
  res1 <- data_admin_init(path1, "pair1", quiet = TRUE)
  res2 <- data_admin_init(path2, "pair1", quiet = TRUE)
...
test_that("gracefully fail to initialise", {
  ...
  testthat::with_mocked_bindings(
    data_authorise_write = function(...) stop("Unexplained error"), {
      msg <- capture_messages(
        try(data_admin_init(path, "pair1"), silent = TRUE))
      expect_match(msg, "Removing data key", all = FALSE)
    })
...
test_that("Work from a subdirectory", {
  ...
  h <- with_dir(sub,
                data_request_access(path_user = pair2, quiet = TRUE))
  ...
  with_dir(sub,
           data_admin_authorise(path_user = pair1, yes = TRUE, quiet = TRUE))
...
test_that("migrate", {
  ...
  data_request_access(path_data, "pair3", quiet = TRUE)
...
test_that("new data sources do not need migrating", {
  path <- tempfile()
  dir.create(path, FALSE)
  data_admin_init(path, "pair1", quiet = TRUE)
