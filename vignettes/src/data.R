...
options(cyphr.user.path=tempfile())
cyphr::ssh_keygen(getOption("cyphr.user.path"), FALSE)
...
## of the many guides available.  A utility function (which simply
## calls `ssh-keygen` for you) is available in this package though.
## You will need to generate a key on each computer you want access
## from.  Don't copy the key around.  If you lose your user key you
...
## cannot be decrypted.  So keep the directory `data_dir/.cyphr`
## safe!  Ideally add it to your version control system so that it
## cannot be lost.  Of course, if you're working in a group, there are
## multiple copies of the data key (each encrypted with a different
## person's personal key) which reduces the chance of total loss.
...
cfg <- cyphr::config_data(data_dir)
...
cyphr::encrypt(saveRDS(iris, filename), cfg)
dir(data_dir)
...
head(cyphr::decrypt(readRDS(filename), cfg))
...
## for this vignette (see minimal example below where this is
## simulated).  Again, typically this user would also not use the
## `cyphr::ssh_keygen` function but use the `ssh-keygen` command
## from their shell.
user2 <- tempfile()
cyphr::ssh_keygen(user2, FALSE)
...
##+ error=TRUE
cfg2 <- cyphr::config_data(data_dir, user2)

## But `user2` is your collaborator and needs access!  What they need
## to do is run:
cyphr::data_request_access(data_dir, user2)
...
cfg2 <- cyphr::config_data(data_dir, user2)
head(cyphr::decrypt(readRDS(filename), cfg2))
...
## Encrypt a file:
cyphr::encrypt(saveRDS(iris, filename), cyphr::config_data(data_dir))

## Request access, on computer 2:
##+ echo=FALSE
oo <- options(cyphr.user.path=user2)
##+ echo=TRUE
...
oo <- options(cyphr.user.path=user2)
##+ echo=TRUE
head(cyphr::decrypt(readRDS(filename), cyphr::config_data(data_dir)))
##+ echo=FALSE
options(oo)
...
## `cyphr.user.path`.  The key may be password protected (and this
