...
## anywhere really).  When run, `encryptr::config_data` will verify
## that it can actually decrypt things.
cfg <- encryptr::config_data(data_dir)

## This object can be used with all the `encryptr` functions (see the
## "encryptr" vignette; `vignette("encryptr")`)
...
## `encryptr::ssh_keygen` function but use the `ssh-keygen` command
## from their shell.
user2 <- tempfile()
